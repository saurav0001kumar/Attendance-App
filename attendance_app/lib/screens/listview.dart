...
import 'package:attendanceapp/screens/editstudent.dart';
        ...
        for (int i = 0; i < data.length; i++)
          Container(
            ...
            child: GestureDetector(
              ...
              onTap: () {
                //   ON TAP
                identityNumber = data[i]["$colSNo"];

                identitySNo = "${data[i]["$colStudentID"]}";
                identityName = "${data[i]["$colName"]}";
                identityUroll = "${data[i]["$colUniversityROLL"]}";
                identityCroll = "${data[i]["$colClassROLL"]}";

                debugPrint(
                    "Tile with ID= $identityNumber is Tapped\n$identityName,$identitySNo,$identityUroll,$identityCroll.");
