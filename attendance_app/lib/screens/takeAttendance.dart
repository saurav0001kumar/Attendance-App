...
//               _scaffoldKey.currentState.openDrawer();
        ...
        body: bodyforAttendance());
  }

  //bodyforAttendance
  Widget bodyforAttendance() {
    for (int j = 0; j < data.length; j++) checkboxSelected.add(false);
...
//                    materialTapTargetSize: MaterialTapTargetSize.shrinkWrap,
                      ...
                      value: checkboxSelected[i],
                      onChanged: (bool cb) {
                        setState(() {
                          checkboxSelected[i] = cb;
                        });

                        debugPrint(
                            "CheckBOX ${data[i]["$colClassROLL"]} Tapped");
                      }),
                  title: Text(
                    "${data[i]["$colName"]}",
                    style:
...
//        mainAxisSize: MainAxisSize.max,
  ...
  Future<Database> getAttendanceList() async {
  ...
  void _createDb(Database dbs, int newVersion) async {
    ...
    if (data.isNotEmpty)
      debugPrint("\n\n${data.length}Attendance Data Found\n\n\n");
    for (int i = 0; i < 1; i++) debugPrint("${data[i]["SNo"]}\n");
  }

/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

  Future<Database> saveAttendance() async {
    // Get the directory path for both Android and iOS to store database.
    var directory = await getExternalStorageDirectory();
  ...
  void _create(Database dbs, int newVersion) async {
    ...
    List l6 = await dbs.rawQuery(
        'SELECT $colTotalAttendance FROM $noteTable WHERE $colSNo = ${data[0]["$colSNo"]}');
    int tot = l6[0]["$colTotalAttendance"];
    tot = tot + 1;

    try {
      for (int k = 0; k < data.length; k++)
        if (checkboxSelected[k] == true) {
          //step1

          List l7 = await dbs.rawQuery(
              'SELECT $colPresentDays FROM $noteTable WHERE $colSNo = ${data[k]["$colSNo"]}');

          List l8 = await dbs.rawQuery(
              'SELECT $colAttendancePercent FROM $noteTable WHERE $colSNo = ${data[k]["$colSNo"]}');
          //step2

          int present = l7[0]["$colPresentDays"];
          present = present + 1;

          var per = l8[0]["$colAttendancePercent"];

          per = present / tot;
          per = per * 100;

          debugPrint("$present and $tot and $per %");

          //step3
          String tempDay = tot.toString();
          String resultDay = "Day$tempDay";
          //Step4
          String dateString = currentDate.toString();
          print("$currentDate");
          //Step5
          await dbs.execute(
              'UPDATE $noteTable SET $resultDay = "$dateString" , $colTotalAttendance = $tot , $colPresentDays = $present , $colAttendancePercent = $per WHERE  $colSNo = ${data[k]["$colSNo"]} ');
...
//      getAttendanceList();
          ...
          String resultDay = "Day$tempDay";
          //Step4
          String dateString = "AB";
          //Step5
          await dbs.execute(
              'UPDATE $noteTable SET $resultDay = "$dateString" , $colTotalAttendance = $tot , $colAttendancePercent = $per WHERE  $colSNo = ${data[k]["$colSNo"]} ');

//      initializeDatabase();
//      getAttendanceList();

          debugPrint("\n\nData Saved as Absent.\n\n\n");
        }
//  String t = "Update Message", c = "Student details updated successfully!";
//  alertDialogDynamic(t, c);
//  fancyAlertDynamic();
      alertSuccessWithIcon();
      ...
      print(error);
      String t = "ERROR!", c = "Attendance not updated.";
      alertDialogDynamic(t, c);
    }
  }
  ...
  void fancyAlertDynamic() {
    FancyAlertDialog.showFancyAlertDialog(
        this.context,
        'Confirm Save Attendance Data ?',
        "Are you sure you want to save Attendance data now? It can't be undone.",
        Colors.indigoAccent,
        icon: Icon(
          Icons.live_help,
          color: Colors.white,
